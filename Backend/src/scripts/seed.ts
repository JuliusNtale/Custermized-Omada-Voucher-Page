/**
 * Database seed: idempotently upsert the initial Internet packages.
 *
 * Compiled to dist/scripts/seed.js and run by the container entrypoint on every
 * start (after `prisma migrate deploy`), so a fresh SQLite file always has the
 * catalog. Also runnable directly: `npm run db:seed`.
 */
import { PrismaClient } from '@prisma/client';
import { INITIAL_PACKAGES } from '../modules/catalog/package-catalog.js';

const prisma = new PrismaClient();

async function main(): Promise<void> {
  for (const pkg of INITIAL_PACKAGES) {
    const data = {
      name: pkg.name,
      price: pkg.price,
      currency: pkg.currency,
      durationSeconds: pkg.durationSeconds,
      speedLimit: pkg.speedLimit,
      downloadLimit: pkg.downloadLimit,
      uploadLimit: pkg.uploadLimit,
      active: pkg.active,
    };
    await prisma.package.upsert({
      where: { id: pkg.id },
      update: data,
      create: { id: pkg.id, ...data },
    });
  }
  console.info(`Seeded ${INITIAL_PACKAGES.length} packages (idempotent).`);
}

main()
  .catch((err) => {
    console.error('Seed failed', err);
    process.exitCode = 1;
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
